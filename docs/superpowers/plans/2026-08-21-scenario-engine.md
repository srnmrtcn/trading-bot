...
- Manual verification after Task 9 (not automated, requires a live Postgres + internet, and enough real market history accumulated by Subsystem A): run `python -m src.main`, wait for at least one hourly job to complete, and confirm the `scenarios` table populates when a real signal fires. Signals may be rare in any given hour — this is expected, not a bug; the automated tests are what verify the logic is correct, not a live run.
